...
function checkVar(varName, value, domElemId) {
	...
	// l'astuce trop belle: on try eval, si ça catch c'est que la variable n'existe pas.
	try {
		const zeVar = eval(varName);

		if (!value || zeVar == value || (typeof zeVar == 'string' && zeVar.toLowerCase() == value.toLowerCase())) {
			checkElem.innerHTML = '&#10003;';
...
/* Check exo 2 */
function checkExo2() {
	checkVar('réponse1', null, 'check_2_0');
...
function checkExo3() {
	...
	checkVar('réponse2', null, 'check_3_2');
